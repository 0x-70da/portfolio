        ...
        <div className="relative w-full h-full z-20 p-7">
          <div className="mb-4.5 pb-3.5 border-b border-alpha-a20">
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <span className="font-heading text-xs tracking-[0.22em] uppercase text-ink-dark">
                {about.header}
              </span>
            </div>
          ...
          <div className="space-y-4">
            {about.bio.map((paragraph, idx) => (
              <p
                key={idx}
                className="font-heading text-wow-2xl leading-loose text-body-muted tracking-wider italic"
              >
                {paragraph}
              </p>
            ))}
          ...
          <div className="flex flex-col gap-2.5 mt-4">
            {factItems.map((fact) => (
              <div key={fact.label} className="flex items-start gap-2.5">
                ...
                <span className="font-heading text-wow-md text-soft tracking-wider leading-[1.6] ml-1">
                  {fact.value}
                </span>
              </div>
            ))}
